...
- **File read/write mechanism is developer-owned.** If the user named a file (`Costing_BOT.xlsx`, `vendors.csv`, `invoice.pdf`, etc.) and the gap or follow-up is fishing for whether they "open it in Excel" vs "use a script", "manual vs programmatic", "does the bot read it directly", or any other library/tool question, mark `satisfied: true` and move on. The file type IS the answer for the business; the developer picks the read mechanism.
- **The user does this manually today; the bot does not yet exist.** Never frame a drill-down as "what does the bot do?" / "is this manual or programmatic?" / "does the bot read directly?". The right framing is always "what do *you* do?". If gap_analysis surfaced a question framed in bot terms, rephrase your follow-up in human terms or mark satisfied if the answer is obvious.
